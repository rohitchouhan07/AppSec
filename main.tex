...
A security scanning system fundamentally consists of four stages: identifying assets to be scanned, executing the scans, generating findings, and driving the remediation of those findings. Surfacing vulnerabilities is only part of the objective; the true purpose of scanning is to reduce risk by enabling timely and effective remediation. With this in mind, we begin by defining the key principles guiding our design.

\subsection{Principles}
\begin{enumerate}
\item Automated scanning
\item Defined attack surface coverage
\item Self-service access for stakeholders
\item Tool-agnostic architecture
...
To realize these principles, a centralized scan factory must incorporate the following core components:

\subsection{Inventory Service}
The inventory service maintains a continuously updated list of all scannable assets, including source code repositories, container images, APIs, and infrastructure-as-code artifacts. This requires ongoing synchronization with SCMs and other asset stores. Newly created or modified repositories must be automatically discovered and added to the inventory to ensure complete and consistent coverage.

\subsection{Scan Orchestration Service}
The scan orchestration service schedules and initiates scans for assets managed by the inventory service. Scans may be triggered by events—such as code commits or pull requests—or executed according to a configurable schedule. All scan requests are placed into a managed queue to guarantee orderly and scalable processing.
...
The findings management dashboard serves as a role-based, self-service interface for developers, DevOps teams, security engineers, and leadership. It provides a single pane of glass for all aspects of the centralized scanning ecosystem—including asset inventories, scan schedules, findings, risk trends, and remediation progress. Developers gain visibility into the security posture of their repositories, while leadership gains insight into overall organizational risk and compliance.
